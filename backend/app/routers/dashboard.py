...
async def mock_topup(
    ...
    allowed_presets = [0.60, 3.00, 6.00, 0.625, 3.125, 6.25, 15.625, 31.25]  # Ekuivalen Rp 10k, 50k, 100k, 250k, 500k
    # Longgarkan presisi toleransi floating point
    is_preset = any(abs(amount - p) < 0.01 for p in allowed_presets)
    if not is_preset:
         raise HTTPException(status_code=400, detail={"error": "invalid_amount", "message": "Top up hanya diizinkan menggunakan nominal simulasi preset: Rp 10k, 50k, 100k, 250k, atau 500k"})
